...
<script type="text/javascript" charset="utf-8">
          $(document).ready( function () {
                    ...
                    $('#add_tax_commodity_form').hide();
                    $('#edit_tax_commodity_form').hide();
                              posnic_table();
                                add_tax_commodity.onsubmit=function()
                                { 
                                  return false;
                                } 
           ...
           $('#dt_table_tools').dataTable({
                                      "bProcessing": true,
				      "bServerSide": true,
                                      "sAjaxSource": "<?php echo base_url() ?>index.php/tax_commodity/data_table",
                                       aoColumns: [  
    ...
    function user_function(guid){
     var tax_commodity=$('#name_'+guid).val();
    <?php if($this->session->userdata['tax_commodity_per']['delete']==1){ ?>
             bootbox.confirm("Are you Sure To Delete This tax_commodity ("+tax_commodity+")", function(result) {
             if(result){
            $.ajax({
                ...
                {
                    if(response){
                           $.bootstrapGrowl('<?php echo $this->lang->line('tax_commodity') ?> '+tax_commodity+' <?php echo $this->lang->line('deleted');?>', { type: "error" });
           ...
           $.bootstrapGrowl('<?php echo $this->lang->line('You Have NO Permission To Delete')." ".$this->lang->line('tax_commodity');?>', { type: "error" });                       
   <?php }
?>
                        }
            function posnic_deactive(guid){
                ...
                {
                    if(response){
                         $.bootstrapGrowl(tax_commodity+' <?php echo $this->lang->line('isdeactivated');?>', { type: "danger" });
                        $("#dt_table_tools").dataTable().fnDraw();
                    }
                }
            });
            ...
            function posnic_active(guid){
             var tax_commodity=$('#name_'+guid).val();
                ...
                {
                    if(response){
                         $.bootstrapGrowl(tax_commodity+' <?php echo $this->lang->line('isactivated');?>', { type: "success" });
                        $("#dt_table_tools").dataTable().fnDraw();
                    }
                }
            });
           ...
           function edit_function(guid){
                       $("#parsley_reg").trigger('reset');
                        <?php if($this->session->userdata['tax_commodity_per']['edit']==1){ ?>
                            $.ajax({                                      
                             url: "<?php echo base_url() ?>index.php/tax_commodity/edit_tax_commodity/"+guid,                      
                             data: "", 
                             dataType: 'json',               
                             success: function(data)        
                             {    
                                 $("#user_list").hide();
                                 $('#edit_tax_commodity_form').show('slow');
