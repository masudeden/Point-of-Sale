...
$( "#date_end_picker" ).datepicker();
     ...
     echo "<tr><td>"; echo form_label($this->lang->line('supplier'));echo "</td><td>"; if(count($srow)>0 ){ ?><select name="supplier" style="width:150"><?php foreach ($srow as $cs_row){     foreach ($sb_row as $csk_row){ if($cs_row->supplier_id == $csk_row->id){ if($it_row->supplier_id ==$csk_row->id ){ ?><option name="<?php echo $csk_row->id ?> "value="<?php echo $csk_row->id?> "selected="selected"> <?php echo $csk_row->company_name  ?> </option><?php }else{ ?><option name="<?php echo $csk_row->id ?> "value="<?php echo $csk_row->id?> "> <?php echo $csk_row->company_name  ?> </option><?php }}}}echo "</select>";?><?php }  echo "</td></tr>";
