...
<script type="text/javascript" charset="utf-8">
    ...
    function user_function(items_category,guid){
    <?php if($_SESSION['items_category_per']['delete']==1){ ?>
...
?>
            ...
            function posnic_deactive(user,guid){
                ...
                {
                    if(response){
                         $.bootstrapGrowl('<?php echo $this->lang->line('items_category') ?> '+user+' <?php echo $this->lang->line('isdeactivated');?>', { type: "danger" });
                        $("#dt_table_tools").dataTable().fnDraw();
                    }
                }
            });
            ...
            function posnic_active(user,guid){
                ...
                success: function(response)
                {
                    if(response){
                          ...
                          $.bootstrapGrowl('<?php echo $this->lang->line('items_category') ?> '+user+' <?php echo $this->lang->line('isactivated');?>', { type: "success" });
                        $("#dt_table_tools").dataTable().fnDraw();
                    }
                }
            });
